...
export function loadRepositoryData(
  ...
  throw new Error(
    "Synchronous repository loading is only available in Node.js.",
  );
}
...
function loadCatalogNodeSync(
  ...
  const processObj = (globalThis as unknown as { process: NodeProcess })
    .process;
...
function loadAllCoursesNodeSync(
  requireFn: (id: string) => unknown,
  dataRoot = "public/data",
): LoadedCourse[] {
  const fs = requireFn("node:fs") as NodeFs;
...
export async function loadCoursesForContext(context: {
  ...
  const academicYear = catalog.academicYears.find(
  ...
  const coursesList = semester?.courses || [];
    ...
    const loaded = await Promise.all(
      coursesList.map(async (c) => {
        const response = await fetch(
          `${baseUrl}data/courses/${academicYearId}/${studyYearId}/${semesterId}/${c.id}.json`,
        );
        if (!response.ok) {
