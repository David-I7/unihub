...
export function IconBadge({
  ...
  const tones = {
    material:
      "bg-[var(--status-upcoming-bg)] text-[var(--status-upcoming-text)]",
    assignment:
      "bg-[var(--status-assignment-bg)] text-[var(--status-assignment-text)]",
    lecture:
      "bg-[var(--status-scheduled-bg)] text-[var(--status-scheduled-text)]",
...
export function ItemTypeIcon({
  ...
  type: "material" | "assignment" | "lecture" | "exam" | string;
  size?: number;
}) {
  switch (type) {
    case "material":
