...
export const STUDY_YEAR_OPTIONS = [
  { value: "YEAR_1" as const, label: "Year 1", slug: "year-1" as const, yearNumber: 1 },
  { value: "YEAR_2" as const, label: "Year 2", slug: "year-2" as const, yearNumber: 2 },
  { value: "YEAR_3" as const, label: "Year 3", slug: "year-3" as const, yearNumber: 3 },
  { value: "YEAR_4" as const, label: "Year 4", slug: "year-4" as const, yearNumber: 4 },
] as const;

export function formatStudyYearName(name: string): string {
  if (!name) return "";
  const clean = name.replace("-", "_").toUpperCase();
  if (clean === "YEAR_1") return "Year 1";
  if (clean === "YEAR_2") return "Year 2";
  if (clean === "YEAR_3") return "Year 3";
  if (clean === "YEAR_4") return "Year 4";
  return name;
}
...
export function slugToStudyYearEnum(slug: string): StudyYearNameEnum {
  ...
  const clean = slug.replace("-", "_").toUpperCase();
  if (clean === "YEAR_1") return "YEAR_1";
  if (clean === "YEAR_2") return "YEAR_2";
  if (clean === "YEAR_3") return "YEAR_3";
  if (clean === "YEAR_4") return "YEAR_4";
  return "YEAR_1";
}
